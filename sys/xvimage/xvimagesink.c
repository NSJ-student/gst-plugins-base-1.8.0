...
enum
{
  ...
  PROP_WINDOW_WIDTH,
  PROP_WINDOW_HEIGHT,
  PROP_WINDOW_POSX,	/* NSJ */
  PROP_WINDOW_POSY,
  PROP_X_DISPLAY,
  PROP_X_PARENT_WINDOW,
  PROP_X_CONTEXT_LOCK,
  PROP_X_FLOW_LOCK,
    ...
    gint width, gint height)
{
  ...
  xwindow = gst_xvcontext_create_xwindow (context, 
	xvimagesink->window_x, xvimagesink->window_y,	/* NSJ */
	width, height, xvimagesink->fullscreen);
...
gst_xv_image_sink_xwindow_update_geometry (GstXvImageSink * xvimagesink)
{
  ...
  {
    gst_xwindow_update_geometry (xvimagesink->xwindow, &xvimagesink->config, xvimagesink->enable_change_window);
    if(xvimagesink->enable_console_debug)	/* NSJ */
    {
	printf("xvimagesink: window size(%d, %d)\n",
		xvimagesink->xwindow->width,
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_DRAW_BORDERS:
      ...
      break;
    case PROP_WINDOW_POSX:
      xvimagesink->window_x = g_value_get_int (value);
      break;
    case PROP_WINDOW_POSY:
      xvimagesink->window_y = g_value_get_int (value);
      break;
    case PROP_WINDOW_FULLSCREEN:
      xvimagesink->fullscreen = g_value_get_boolean (value);
      break;
   case PROP_CONSOLE_DEBUG:
      xvimagesink->enable_console_debug = g_value_get_boolean (value);
      break;
   case PROP_CHANGE_SIZE:
      xvimagesink->enable_change_window = g_value_get_boolean (value);
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_WINDOW_HEIGHT:
      ...
      break;
    case PROP_WINDOW_POSX:
      g_value_set_int(value, xvimagesink->window_x);
      break;
    case PROP_WINDOW_POSY:
      g_value_set_int(value, xvimagesink->window_y);
      break;
    case PROP_X_DISPLAY:
      g_value_set_pointer(value, xvimagesink->context->disp);
      break;
    case PROP_X_PARENT_WINDOW:
      g_value_set_pointer(value, &xvimagesink->xwindow->win);
      break;
    case PROP_X_CONTEXT_LOCK:
      g_value_set_pointer(value, &(xvimagesink->context->lock));
      break;
    case PROP_X_FLOW_LOCK:
      g_value_set_pointer(value, &(xvimagesink->flow_lock));
      break;
    case PROP_WINDOW_FULLSCREEN:
      g_value_set_boolean(value, xvimagesink->fullscreen);
      break;
   case PROP_CONSOLE_DEBUG:
      g_value_set_boolean(value, xvimagesink->enable_console_debug);
      break;
   case PROP_CHANGE_SIZE:
      g_value_set_boolean(value, xvimagesink->enable_change_window);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gst_xv_image_sink_class_init (GstXvImageSinkClass * klass)
{
  ...
  /** NSJ
   * GstXvImageSink:window-posx
   *
   * Actual X position of the video window.
   */
  g_object_class_install_property (gobject_class, PROP_WINDOW_POSX,
      g_param_spec_int ("window-pos-x", "window-pos-x",
          "X Position of the window", -10000, 10000, 0,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  /** NSJ
   * GstXvImageSink:window-posy
   *
   * Actual Y position of the video window.
   */
  g_object_class_install_property (gobject_class, PROP_WINDOW_POSY,
      g_param_spec_int ("window-pos-y", "window-pos-y",
          "Y Position of the window", -10000, 10000, 0,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  /** NSJ
   * GstXvImageSink:object
   *
   * GstXvImageSink object.
   */
  g_object_class_install_property (gobject_class, PROP_X_DISPLAY,
      g_param_spec_pointer ("x-display", "x-display", "display object of window", 
          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_X_PARENT_WINDOW,
      g_param_spec_pointer ("x-parent-window", "x-parent-window", "parent window object", 
          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_X_CONTEXT_LOCK,
      g_param_spec_pointer ("context-lock", "context-lock", "xvimagesink context lock", 
          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_X_FLOW_LOCK,
      g_param_spec_pointer ("flow-lock", "flow-lock", "xvimagesink flow lock", 
          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_WINDOW_FULLSCREEN,
      g_param_spec_boolean ("fullscreen", "fullscreen", "Make fullscreen window",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_CONSOLE_DEBUG,
      g_param_spec_boolean ("console-debug", "console_debug", "Enable console debug",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
  g_object_class_install_property (gobject_class, PROP_CHANGE_SIZE,
      g_param_spec_boolean ("change-size", "change-size", "Enable automately change window wize",
          FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
