...
gst_xvcontext_create_xwindow (GstXvContext * context, gint x, gint y, 
			gint width, gint height, gboolean fullscreen)
{
  GstXWindow *window;
  Atom wm_delete;
...
//  if(fullscreen != True)
  {
	Screen * screen;
	screen = ScreenOfDisplay(context->disp, 0); 

	window->render_rect.w = screen->width;
	window->render_rect.h = screen->height;
  ...
  window->win = XCreateSimpleWindow (context->disp,
      context->root, 0, 0, window->width, window->height, 0, 0, context->black);
  XMoveWindow(context->disp, window->win, x, y);	/* NSJ */
  ...
  {
	Atom wm_state = 0;
	Atom wm_screen[2] = {0, 0};

	wm_state = XInternAtom ( context->disp, "_NET_WM_STATE", True );
	wm_screen[0] = XInternAtom ( context->disp, "_NET_WM_STATE_FULLSCREEN", True );
	wm_screen[1] = XInternAtom ( context->disp, "_NET_WM_STATE_ABOVE", True );
	XChangeProperty( context->disp, window->win, wm_state, XA_ATOM, 32, 
			PropModeReplace, (unsigned char *)wm_screen, 1);
  }
...
void
gst_xwindow_update_geometry (GstXWindow * window, GstXvContextConfig * config, gboolean change_size)
{
  XWindowAttributes attr;
  GstXvContext *context;
  Display * dpy;
  ...
  if(change_size == True)
  {
	dpy = XOpenDisplay(config->display_name);
	if(dpy != NULL)
	{
		/* Update the window size by screen resolution */
		scr = DefaultScreen(dpy);
		d_width = DisplayWidth(dpy, scr);
		d_height = DisplayHeight(dpy, scr);
		if((d_width != window->width) || (d_height != window->height))
		{
			XResizeWindow(context->disp, window->win, d_width, d_height);
  			XMoveWindow(context->disp, window->win, 0, 0);	/* NSJ */
			XSync(context->disp, FALSE);
		}
  	}
  }
...
/*
  printf("update geometry: pos(%d, %d), window size (%d, %d), render rect(%d, %d)\n",
